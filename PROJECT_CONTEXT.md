...
| Test count at generation | **668 collected / 649 passing / 19 skipped** (12 PyRosetta benchmark + 5 ColabFold accuracy-benchmark tests skip without their opt-ins/env; 2 opt-in live e2e tests skip too) |
...
| `test_sequence_viewer.py` | 9 | Sequence-Viewer layer (no live ChimeraX): SCF writer format + RGB clamping + resnum→0-based alignment col (incl. non-1 start & gaps) + seq_index + skip-unknown-resnum + empty-file; `build_scf_runscript` writes the `new_region` loader and returns `runscript "<py>"`; Part A command construction (`sequence chain #M/CH`, `mousemode left select/rotate`); CamSol `analyze()` appends the `sequence chain` + `runscript` commands and writes a non-empty `.scf` (hermetic via monkeypatched cache dir) |
