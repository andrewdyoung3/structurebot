...
| Test count at generation | **624 collected / 605 passing / 19 skipped** (real run on merged `main`; 12 PyRosetta benchmark + 5 ColabFold accuracy-benchmark tests skip without their opt-ins/env; 2 opt-in live e2e tests skip too) |
...
### ⚠️ ColabFold DEFAULT (5-model) fold blocked by WSL2 host-memory cap (diagnosed 2026-06-01)
**The worker failure-path error reporting is FIXED** (2026-06-01, `fix/colabfold-default-fold`): it previously did `error = log[-3000:]` on `stdout+stderr` concatenated, so a benign trailing TF/oneDNN stderr line buried the real traceback; now it surfaces the last 60 lines of **stdout AND stderr separately labelled** (+ `returncode`, `stdout_tail`, `stderr_tail`) and the bridge propagates it (no 300-char clip). Canonical formatter `_format_worker_failure()` + 3 unit tests.

With the real error visible, the **default 5-model / 3-recycle fold (and ANY config beyond the 1-model/1-recycle `quick` preset) reproducibly SIGSEGVs (exit -11)**. Root cause: XLA tries to allocate **~48 GiB of pinned host memory** (≈ 4× the 12 GB GPU VRAM — a host-offload staging buffer), backs off 48→43→…→16 GiB, but **WSL2 has only ~15 GiB** (default ~50% of the 31.7 GB host; no `~/.wslconfig`), so every retry exceeds the cap → segfault. Isolation: **5-model/1-recycle FAILS and 1-model/3-recycle FAILS** (identical signature) — it's neither knob alone; only `quick` (1/1) fits in 15 GiB and succeeds. **NOT a bridge bug** (args `--num-models 5 --num-recycle 3` are correct). **Fix requires an env/default change (STOP-and-report territory) — NOT applied here.** Options, best first: (1) raise WSL2 memory via `~/.wslconfig` `[wsl2]\nmemory=24GB` + `wsl --shutdown` — XLA's backoff bottoms out at ~16 GiB which just exceeds the current 15 GiB cap, so giving it ~24 GiB (within the 31.7 GB host) is the most likely unblock; (2) an XLA env flag in the worker to cap/disable the large pinned-host pre-allocation (needs the exact flag identified + live-tested); (3) drop the default to `quick` (1/1) — the only confirmed-working config — at a fold-quality cost. Until resolved, **`quick=True` (1/1) is the working path** for small monomers.
