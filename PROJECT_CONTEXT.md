...
| Test count at generation | **674 collected / 655 passing / 19 skipped** (12 PyRosetta benchmark + 5 ColabFold accuracy-benchmark tests skip without their opt-ins/env; 2 opt-in live e2e tests skip too) |
...
| `test_translator.py` | 17 | `CommandTranslator` API mock, prompt caching structure, history management, **over-eager-refusal handling** (benign design request not flagged — retry rescues a transient empty response; `_call_api` retries once then raises a transparent `RefusalError`; scope-framing in the prompt); **Chimera-1 zone-syntax guard** (`_sanitize_zone_syntax` rewrites `zone <spec> <dist>` and the parenthesised `(zone …)` → `<spec> :<<dist>`, leaves valid `:<`/`volume zone`, drops unrewritable; end-to-end through `translate()`); **hide/show targets the representation** (`hide #1/B cartoon`, not bare `hide`); prompt documents `:<`/`@<` + `info residues sel` + hide-cartoon |
