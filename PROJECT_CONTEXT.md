...
Bottom line: use PyRosetta ddG for **ranking + sign** (good), aggregate by **median**. A validation tier (median-of-N at higher cycle counts) reduces but does **not** eliminate the magnitude bias — **disclose absolute ddG as approximate** until a full 2LZM-panel re-run at the chosen N+cycles confirms RMSE.
...
|----------|------|-----------|
| 1 | **Multi-trajectory ddG (median aggregation + validation tier)** | **Design confirmed** by the aggregation + convergence diagnostics: aggregate by **MEDIAN** (MIN refuted), and use **more relax cycles (e.g. 8+8) for a validation tier** — both move medians toward experiment and shrink the ~8 kcal/mol spread, but only ~half the magnitude gap closes, so **ship with a "ranking reliable, magnitudes approximate" disclosure**. Implementation: add `ROSETTA_NUM_TRAJECTORIES` (default 1) running N trajectories→median; wire the existing `relax_cycles` param to a validation-tier setting (keep production fast/single-trajectory — 8+8 trajectories are ~3–5 min each, not for interactive scans). **Confirmation step:** re-run the full 2LZM panel at the chosen N+cycles to check RMSE before claiming calibration. `rosetta_bridge.py` now free (diagnostic finished) |
...
- **Immediate next item**: implement the multi-trajectory ddG (build-queue item 1) with the **confirmed design** — `ROSETTA_NUM_TRAJECTORIES` (default 1) → median, plus a validation tier at higher `relax_cycles` (keep production single-trajectory). Then the crystal-water cleanATOM fix. Both edit `rosetta_bridge.py` (now unblocked). Re-run the full 2LZM panel afterwards to confirm RMSE before claiming calibration.
