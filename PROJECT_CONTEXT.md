...
| 1 | **Selective / movable water handling — do not default to preserve-all-static** | Current default `ROSETTA_STRIP_WATERS=False` preserves **ALL ~216 crystallographic waters as static HOH**; static waters act as immovable spheres → clash-driven over-destabilization on non-buried surface/cavity mutations (§7 Water handling). Best practice: strip bulk/surface (the `ref2015` `lk_ball` implicit solvent already models surface desolvation), keep **only buried/bridging** waters, and **movably** (Rosetta-ICO/ECO; Pavlovicz, Park & DiMaio 2020). After the preserve-path 2LZM re-validation (item 1), move to **selective buried-only** preservation or revert the default to **strip** with preserve as opt-in |
