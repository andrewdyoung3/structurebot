...
| Test count at generation | **643 collected / 624 passing / 19 skipped** (12 PyRosetta benchmark + 5 ColabFold accuracy-benchmark tests skip without their opt-ins/env; 2 opt-in live e2e tests skip too) |
...
| `test_translator.py` | 9 | `CommandTranslator` API mock, prompt caching structure, history management, **zone-operator guidance** (within-N-Å → `:<`, not Chimera-1 `zone`) |
...
|------|-------|-------------|
| 2026-06-01 | 643 (624✓/19s) | fix(translator): **emit ChimeraX zone OPERATORS, not Chimera-1 `zone`** (`fix/translator-zone-syntax`). "residues within N Å of chain X" was translating to `zone #1/B 4.5` — Chimera-1 syntax, INVALID in ChimeraX 1.11 (empty selection). Added rule 16 to the `_STATIC_SYSTEM` prompt: use `:<` (whole-residue zone) / `@<` (atom zone) with `&`/`~`, e.g. chain-A residues within 4.5 Å of chain B → `select #1/B :<4.5 & #1/A` + `info residues sel`; with 2–3 examples (interface, near a ligand `:MK1 :<4 & ~:MK1`, atom zone `@<3`) so it generalises; `zone ...` is explicitly flagged invalid/forbidden. Test: +1 in `tests/test_translator.py` (real asserts: the static prompt teaches `:<`/`@<` for within-N-Å and flags `zone` invalid). Non-benchmark suite **624 passed + 2 skipped**. NOT merged. |
