...
|----------|------|-----------|
| low (conditional) | **Preserve-path 2LZM validation-tier re-run** — *only required if preserve is promoted to default or made selective* | The committed validation-tier panel (RMSE 2.73, r +0.50, sign 90%; §7) was measured on the **stripped-water** path, which is **again the default** (reverted 2026-05-31), so the committed numbers describe the default and no re-run is needed for current behaviour. This becomes necessary **only** if `ROSETTA_STRIP_WATERS=False` (preserve) is promoted back to default, or when selective buried-only preservation lands — then re-run the full 10-mutation 2LZM panel at the validation tier on that path and compare. ~4.5 h via `scripts/validate_2lzm_panel.py` (preserve-path uses a fresh `_wat` relax-cache namespace, so it won't collide with the stripped-water cache) |
| ~~1~~ ✅ | ~~**Verify/fix REU→kcal/mol scaling (2.94 factor)**~~ — ✅ **AUDIT COMPLETE (2026-05-30); verdict: leave uncalibrated** | Active path is a manual `ref2015` torsion-space delta (`rosetta_bridge.py:1449-1464`) reported as raw REU with **no scaling** (factor 1.0). Park's 2.94 is cartesian_ddg-specific and **over-corrects** here (÷2.94: MAE 2.59→1.24 but bias flips negative, large-cavity mutations wrecked). **DECISION: add no scaling/fitted factor** (2.94 over-corrects; a 10-point fit would overfit). Magnitudes stay uncalibrated; ranking/sign unaffected (monotonic). Full verdict in §7 Calibration |
| backlog | **Adopt canonical cartesian_ddg for calibrated magnitudes (LATER)** | **Only if** absolute ΔΔG magnitudes ever become a requirement: replace the manual torsion-space delta with the canonical protocol — cartesian relax + `ref2015_cart` + multi-round averaging + the published ~2.94 REU→kcal/mol factor. **Ranking/sign is unaffected by scaling**, so this is unnecessary for screening/ranking use. Do **not** instead patch a scaling factor onto the current torsion-space delta (2.94 over-corrects it) |
| 1 | **Selective / movable buried-only water handling** (real future fix) | The default is now **strip** (`ROSETTA_STRIP_WATERS=True`, reverted 2026-05-31) and preserve is opt-in — but the opt-in preserve path preserves **ALL ~216 crystallographic waters as static HOH**, and static waters act as immovable spheres → clash-driven over-destabilization on non-buried surface/cavity mutations (§7 Water handling). The principled fix (better than either strip-all or preserve-all-static): keep **only buried/bridging** waters — the `ref2015` `lk_ball` implicit solvent already models surface desolvation — and treat them as **movable** (Rosetta-ICO/ECO; Pavlovicz, Park & DiMaio 2020). This would capture the T26A-style buried-water benefit without the surface-mutation penalty, and would then warrant a preserve/selective-path 2LZM re-validation |
| 2 | ~~**Crystal-water cleanATOM fix**~~ — ✅ **COMPLETE (2026-05-30, commit 3318bc9)** | Added `ROSETTA_STRIP_WATERS` + preserve code path; worker re-appends crystallographic HOH that cleanATOM strips, relax cache namespaced by mode. Validated live on **Barnase T26A**: strip −0.34 (wrong sign) → preserve +3.47 (correct, exp +1.3), +3.80 shift, 216 HOH reached the pose. **Default note:** commit 3318bc9 shipped default `False`=preserve, but the default was **reverted to `True`=strip on 2026-05-31** (preserve-all-static is a Rosetta anti-pattern; only the strip path is panel-validated). Preserve remains available opt-in; proper fix tracked as the selective/movable buried-only item above |
...
- **Immediate next item**: items are now design-loop work (ColabFold/RFdiffusion, Build Queue §9 items 4–5) or the selective/movable water fix; no PyRosetta re-run is pending for current default behaviour.
