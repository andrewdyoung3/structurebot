...
2. **Calibration (KNOWN LEVER — under audit).** ref2015 cartesian_ddg REU famously requires a **~2.94 scaling factor → kcal/mol** (Park 2016), and that factor is **protocol-specific**. Our systematic over-prediction (panel mean signed error ≈ **+1.3 kcal/mol**; e.g. L99A +7.95 vs exp +5.0) **may be partly a calibration artifact**. **Status: under audit** — verdict to be recorded here once known (see Build Queue: verify/fix REU→kcal/mol scaling).
...
| 1 | **Verify/fix REU→kcal/mol scaling (2.94 factor)** | Audit the REU→kcal/mol conversion (§7 Calibration). Published ~2.94 factor (Park 2016) is **cartesian_ddg-specific**, and our active path is a manual `ref2015` torsion-space delta (`rosetta_bridge.py:1449-1464`) reported as raw REU with no scaling. Determine whether the systematic +1.3 kcal/mol over-prediction is a calibration artifact and record the verdict in §7. Scaling is irrelevant to ranking/sign (monotonic) — only absolute magnitudes are affected |
